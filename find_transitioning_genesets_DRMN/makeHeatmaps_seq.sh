...
HEATMAPPER=$3
MARKCOLORMAP="7:(255,255,255);10:(252,146,114);14:(103,0,13)"
echo "INFO: Current color map goes from log(read count)=7 to log(read count)=12"
...
cat $OUTDIR/ordered_clusterset_means.txt| $HEATMAPPER -vC="-2:(250,250,250);-1:(255,255,255);0:(90,180,172);$HALFCID:(245,245,245);$MAXCID:(216,179,101) $MARKCOLORMAP" -vStrokeC="-" -vStrokeSC="black" -vFontSize=8 -vL="moduleIDs marklevel"> $FIGDIR/ordered_clusterset_means.svg
...
do
	...
	FIGNAME=$FIGDIR/$FIGNAME
 	...
 	cat $FNAME | $HEATMAPPER -vC="-2:(250,250,250);-1:(255,255,255);0:(90,180,172);$HALFCID:(245,245,245);$MAXCID:(216,179,101) $MARKCOLORMAP" -vStrokeC="-" -vStrokeSC="black" -vFontSize=6 -vL="moduleID marklevel" > $FIGNAME
done
