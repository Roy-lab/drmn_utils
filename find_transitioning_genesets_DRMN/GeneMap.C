...
/*
CMINT: An algorithm to cluster functional omics data from multiple cell types
...
GeneMap::~GeneMap()
{
	...
	{
		map<string,STRINTMAP*>* grp=gIter->second;
		for(map<string,STRINTMAP*>::iterator hIter=grp->begin();hIter!=grp->end();hIter++)
		{
			hIter->second->clear();
			delete hIter->second;
		}
