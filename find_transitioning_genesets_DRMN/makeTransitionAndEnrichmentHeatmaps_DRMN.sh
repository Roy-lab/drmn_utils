...
if [ -d $FIGDIR ]
then
	echo "Please remove the figs directory in $OUTDIR (overwriting now)"
fi
mkdir $FIGDIR
...
$enrich ${OUTDIR}/all_genesets.txt <(cut -f1 ${OUTDIR}/all_clusterassign.txt) $cisbp 0.05 ${outpref} persg
# make a 3-col file from those results
sed 's/Cluster/clusterset/' ${outpref}_details.txt | awk '{printf "%s||8\t%s||8\t%.2f|4\n", $1, $2, -1*log($4)/log(10)}' > ${outpref}_for_hmawk.txt

cat $OUTDIR/ordered_clusterset_means.txt <(echo "|- enrichVspacer|Spacer") ${outpref}_for_hmawk.txt | $HEATMAPPER -vC="$modmap" -vStrokeC="-" -vStrokeSC="black" -vFontSize=8 -vL="moduleIDs expr size -log10qval" -vD="-2"> $FIGDIR/ordered_clusterset_means.svg

convert ${FIGDIR}/ordered_clusterset_means.svg ${FIGDIR}/ordered_clusterset_means.png
...
do
	...
	FIGNAME=$FIGDIR/$FIGNAME 	
 	cat $FNAME | $HEATMAPPER -vD="-2" -vC="$modmap" -vStrokeC="-" -vStrokeSC="black" -vFontSize=8 -vL="moduleID expr size" > $FIGNAME
done
